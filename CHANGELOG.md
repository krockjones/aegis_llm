...
- **`docs/ALPHA_PUBLISH_CHECKLIST.md`**: maintainer-only publish checklist removed from the repository.
...
- **Copyright / packaging:** [LICENSE](LICENSE) appendix and [pyproject.toml](pyproject.toml) `project.authors` now credit **Peter Kinson** instead of **AegisOneConnect, LLC**.
- **Repository:** [.gitignore](.gitignore) ignores the full **`temp_docs/`** directory tree (local drafts and planning notes).
- **License:** the project is released under the **Apache License 2.0** ([LICENSE](LICENSE)); [README.md](README.md) and [pyproject.toml](pyproject.toml) updated from proprietary wording (initial Apache metadata listed **AegisOneConnect, LLC**; see current appendix and authors above).
- **Alpha pre-publish docs:** [README.md](README.md) reorganized for **public alpha** (first-screen order, community one-liner, **Alpha release scope**, canonical **Docker Compose** quickstart, **Alternatives** for local Python / uvicorn / Guard-only Docker). [GETTING_STARTED.md](docs/GETTING_STARTED.md) labels Path A as canonical and Path B as alternative. [SECURITY_POSTURE.md](docs/SECURITY_POSTURE.md) clarifies **warnings vs enforcement**. [INTEGRATION_OPEN_WEBUI.md](docs/INTEGRATION_OPEN_WEBUI.md) states doc role. [pyproject.toml](pyproject.toml) description tightened. Optional GitHub **bug report** issue template, **`jq`** guard in `examples/curl_examples.sh`, compose comments for Tier-C env placeholders, and **`.gitignore`** entries for `.env` / `.env.*`.
